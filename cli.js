...
const USAGE = `canva-magic-layers — photo in, all editable layers out
  ...
  node cli.js mcp                          Run as an MCP server for your agent (Claude Code, Codex, Hermes, OpenClaw)
...
async function run(argv, deps = require("./canva")) {
  ...
  try {
    if (cmd === "mcp") require("./mcp").serve(deps);
    else if (cmd === "status") console.log((await status()) ? "Signed in to Canva. ✓" : "Not signed in — run `node cli.js login`.");
