...
each layer — to agents, fully hands-off after a one-time login.
...
**Node 22+** and an installed **Chrome / Edge / Brave** — no browser is bundled or
downloaded, and there is no `npm install`.

## Setup

Requires Node 22+ and Chrome/Edge/Brave. A Canva **Pro/Teams** account is
recommended (transparent export and the feature are gated).
...
Sign in with Canva email / magic-link rather than Google SSO (Google blocks
automated browsers). Login is the only human step: it establishes the session in a
persistent Chrome profile (`~/.canva-magic-layers-profile`), which **is** the
credential — no API keys, no cookie files. The session lasts a while; re-run `login`
when `status` reports signed-out.
...
Offline tests mock the browser/network and pass on any machine. Live end-to-end
checks are gated and skipped by default; unlock with `CANVA_LIVE=1` (plus
...
- Input: a single **PNG/JPG**. Magic Layers needs a **reasonably high-resolution**
  image — it rejects small ones; on a too-small input `decompose` surfaces Canva's
  own "too small" message rather than failing silently.
- Magic Layers is an async AI job (~30-90s) and consumes the account's AI allowance.
...
- **API-free alternative** (no browser, no ToS concern): `fal-ai/qwen-image-layered`
  (~$0.05/run) returns N transparent layers with inpainting — lower fidelity than
  Canva on text/complex art.
```
